...
TABLES: ddldependency,dd25t,dd26s, dd03l.

DATA: gt_25t  TYPE  STANDARD TABLE OF dd25t,
      gs_25t  TYPE dd25t,
      gt_25t2 TYPE  STANDARD TABLE OF dd25t,
      gs_25t2 TYPE dd25t.
...
SELECT-OPTIONS: s_view FOR dd25t-ddtext NO INTERVALS ,
                ...
                s_field FOR dd03l-fieldname NO INTERVALS.

PARAMETERS: p_flag TYPE flag."alan ile arama kontrolü
* At Selection Screen
AT SELECTION-SCREEN.
...
START-OF-SELECTION.
  ...
  IF p_flag IS NOT INITIAL.
    SELECT t~viewname, ddtext, objecttype, fieldname
    FROM ddldependency AS b INNER JOIN dd25t AS t ON t~viewname = b~objectname INNER JOIN dd03l AS l
    ON b~objectname = l~tabname AND b~state = l~as4local
    INTO TABLE @DATA(lt_final2) WHERE t~ddlanguage = 'E' AND l~fieldname IN @s_field AND t~ddtext IN @s_view.

    SORT lt_final2 BY viewname ASCENDING.

    DELETE ADJACENT DUPLICATES FROM lt_final2 COMPARING viewname.

    DESCRIBE TABLE  lt_final2 LINES DATA(l_tabix2).

    DATA: l_count2 TYPE c LENGTH 7.
    l_count2 = l_tabix2.

    CONCATENATE 'Total no. of CDS found' l_count2 INTO DATA(msg2).
    MESSAGE msg2 TYPE 'I'.
* Call Factory Method
